...
# get LedisDB
RUN wget -O ledisdb-src.tar.gz "https://github.com/hello2dj/ledisdb/archive/v$LEDISDB_VERSION.tar.gz" && \
    tar -zxf ledisdb-src.tar.gz && \
    mkdir -p /go/src/github.com/ledisdb/ && \
    mv ledisdb-$LEDISDB_VERSION /go/src/github.com/hello2dj/ledisdb
...
RUN wget -O leveldb-src.tar.gz "https://github.com/google/leveldb/archive/$LEVELDB_VERSION.tar.gz" && \
    ...
    patch -p0 < /go/src/github.com/hello2dj/ledisdb/tools/leveldb.patch && \
...
RUN export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/build/lib:/usr/lib && \
    ...
    mkdir -p /build/bin && \
    rm -rf /build/bin/* && \
    cd /go/src/github.com/hello2dj/ledisdb && \
...
COPY --from=builder /go/src/github.com/hello2dj/ledisdb/config/config-docker.toml /config.toml
